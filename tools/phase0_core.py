...
def sanitize_text(text: str) -> str:
    ...
    sanitized = _SECRET_ASSIGNMENT.sub(lambda m: f"{m.group('prefix')}[REDACTED]", sanitized)
    return sanitized
...
def run_command(argv: Sequence[str], timeout_s: float = DEFAULT_TIMEOUT_S) -> CommandObservation:
    ...
    try:
        ...
        return CommandObservation(
            argv=tuple(argv), status="timeout", returncode=None, stdout="", stderr="",
            duration_ms=int((time.monotonic() - started) * 1000), truncated=False,
        )
    except FileNotFoundError:
        return CommandObservation(
            argv=tuple(argv), status="not_found", returncode=None, stdout="", stderr="",
            duration_ms=int((time.monotonic() - started) * 1000), truncated=False,
        )
    except PermissionError:
        return CommandObservation(
            argv=tuple(argv), status="permission_denied", returncode=None, stdout="", stderr="",
            duration_ms=int((time.monotonic() - started) * 1000), truncated=False,
        )
